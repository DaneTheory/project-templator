...
exports.getTemplates = co.wrap(function* () {
    ...
    return yield Promise.all([
        ...
        globby(['*'], { cwd: reposDir }).then((folders) => folders.filter((folder) => /%%/.test(folder)))
    ]);
});
exports.readTemplatePkg = function (name) {
    return require(path.join(configUtils.packagesDir, 'node_modules', name, 'package.json'));
};
exports.updateNotify = function (pkg) {
    const notifier = update({ pkg });
    if (notifier.update) {
        process.on('exit', () => {
            console.log(boxen('Template update available ' +
